...
#define dbName @"ArticleDateBase"
#define tbName @"ArticleTable"

@implementation ArticleSQL
...
+ (void)insertArticleModel:(IndexModel *)model{
    ...
    if ([self existArticleModel:model]==TRUE) {
        return;
    }else{
...
+ (void)deleteArticleModel:(IndexModel *)model{
    ...
    if ([self existArticleModel:model]) {
...
+ (BOOL)existArticleModel:(IndexModel *)model{
    ...
    NSMutableArray *newArray = [self ArticleList];
